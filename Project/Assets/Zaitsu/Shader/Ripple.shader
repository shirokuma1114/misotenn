Shader "Custom/Ripple" {
	Properties
	{
		...
		_LoopTime("LoopTime", Range(0.1, 10.0)) = 1.5
	...
	#include "UnityCG.cginc"
				...
				sampler2D _MainTex;
				fixed4 _Color;
				float _StartTime,_Width,_StartWidth, _AnimationTime, _LoopTime, _isAlpha,_isColorShift;
				fixed4 frag(v2f IN) : SV_Target
				{
					fixed4 color = tex2D(_MainTex, IN.texcoord)*_Color;
					float2 pos = (IN.texcoord - float2(0.5,0.5)) * 2; //-1~1 ïù2
