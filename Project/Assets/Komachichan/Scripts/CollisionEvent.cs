    ...
    public void Update()
    {
        ...
        if (_phase == Phase.INIT)
        {
            if (!_messageWindow.IsDisplayed)
            {
                ...
                {
                    _messageWindow.SetMessage(_targets[_targetIndex].Name + "ÇÕêgÇéÁÇÁÇÍÇƒÇ¢ÇΩÅI", _owner);
                    _phase = Phase.END;
                }
                else if (_targets[_targetIndex].Souvenirs.Count == 0)
                {
                    _messageWindow.SetMessage(_targets[_targetIndex].Name + "ÇÕÇ®ìyéYÇéùÇ¡ÇƒÇ¢Ç»Ç©Ç¡ÇΩÅI", _owner);
                    _phase = Phase.END;
                }
                else
                {
