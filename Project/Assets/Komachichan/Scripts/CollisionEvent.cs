...
public class CollisionEvent
{
    ...
    public CollisionEvent(CharacterBase owner, List<CharacterBase> targets)
    {
        ...
        _messageWindow.SetMessage("è’ìÀÅI", owner);
    ...
    public void Update()
    {
        ...
        if (_phase == Phase.INIT)
        {
            if (!_messageWindow.IsDisplayed)
            {
                ...
                {
                    _messageWindow.SetMessage(_targets[_targetIndex].Name + "ÇÕÅ@éÁÇÁÇÍÇƒÇ¢ÇΩÅI", _owner);
                    _phase = Phase.END;
                }
                else if (_targets[_targetIndex].Souvenirs.Count == 0)
                {
                    _messageWindow.SetMessage(_targets[_targetIndex].Name + "ÇÕÅ@Ç®ìyéYÇÅ@éùÇ¡ÇƒÇ¢Ç»Ç©Ç¡ÇΩÅI", _owner);
                    _phase = Phase.END;
                }
                else
                {
        ...
        if(_phase == Phase.SELECT)
        {
            ...
            if (true /* ëIë or ÉâÉìÉ_ÉÄÇ≈Ç®ìyéYÇ™ëIëÇ≥ÇÍÇΩ */)
            {
                ...
                _messageWindow.SetMessage(_targets[_targetIndex].Name + "ÇÃ\nÇ®ìyéYÅ@" + souvenir.Name + "ÇÅ@Ç¢ÇΩÇæÇ¢ÇΩÅI", _owner);
